    ...
    return {
        onExcelUpload: function (oEvent) {
            ...
            if (!this.pDialog) {
                Fragment.load({
                    id: "excel_upload",
                    name: "com.gsitm.pkg.co.zgspkgco0030.ext.fragment.ExcelUpload",
                    type: "XML",
                    controller: this
                }).then((oDialog) => {
        ...
        onTempDownload: function (oEvent) {
            ...
            var oBuilding = oModel.getServiceMetadata().dataServices.schema[0].entityType.find(x => x.name === 'CostingTypeMasterType');

            var propertyList = ['Account', 'Materialclass', 'Materialtype', ];
        ...
        callOdata: function (fnResolve, fnReject) {
            ...
            this.excelSheetsData[0].forEach((value, index) => {
                ...
                payload = {
                    "Account": String(value["계정명"]),
                    "Materialclass": String(value["평가클래스"]),
                    "Materialtype": String(value["이동평균(V)/표준(S)"]),
                };
                // calling the odata service
                oModel.create("/CostingTypeMaster", payload, {
                    success: (result) => {
                        console.log(result);
