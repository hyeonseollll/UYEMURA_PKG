    ...
    return {
        onExcelUpload: function(oEvent) {
            ...
            if (!this.pDialog) {
                Fragment.load({
                    id: "excel_upload",
                    name: "com.gsitm.pkg.co.zgspkgco0010.ext.fragment.ExcelUpload",
                    type: "XML",
                    controller: this
                }).then((oDialog) => {
        ...
        onTempDownload: function (oEvent) {
            ...
            var oBuilding = oModel.getServiceMetadata().dataServices.schema[0].entityType.find(x => x.name === 'GLMTMasterType');

            var propertyList = ['MaterialvaluationClass', 'ClassName', 'GLAccount', 'GLAccountName'];
            ...
            XLSX.writeFile(wb, '수불부 GL-평가클래스 기준정보.xlsx');
        ...
        callOdata: function (fnResolve, fnReject) {
            ...
            this.excelSheetsData[0].forEach((value, index) => {
                ...
                payload = {
                    "MaterialvaluationClass": String(value["평가클래스"]),
                    "ClassName": String(value["평가클래스명"]),
                    "GLAccount": String(value["G/L계정"]),
                    "GLAccountName": String(value["G/L계정명"])
                };
                // calling the odata service
                oModel.create("/GLMTMaster", payload, {
                    success: (result) => {
                        console.log(result);
                        var oMessageManager = sap.ui.getCore().getMessageManager();
                        var oMessage = new sap.ui.core.message.Message({
                            message: "평가클래스" + result.MaterialvaluationClass + "가 생성되었습니다.",
